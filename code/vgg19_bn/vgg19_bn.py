...
class Vgg19_bn:
    ...
    def build(self, rgb):
        ...
        self.conv1_1 = self.conv_layer(bgr, "conv1_1", "features.0", "features.1")
        self.conv1_2 = self.conv_layer(self.conv1_1, "conv1_2", "features.3")
        
        self.pool1 = self.max_pool(self.conv1_2, 'pool1')
        
        self.conv2_1 = self.conv_layer(self.pool1, "conv2_1", "features.7")
    ...
    def conv_layer(self, bottom, name, feature_name, bn_name):
        with tf.variable_scope(name):
            ...
            conv_biases = self.get_bias(npy_name)
            bias = tf.nn.bias_add(conv, conv_biases)
            
            # TODO add BN
            beta = get_bias(bn_name)
            gamma = get_weight(bn_name)
            
            mean = get_bn_mean(bn_name)
            variance = get_bn_var(bn_name)
